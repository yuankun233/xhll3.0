    ...
    <view class="container">
        ...
        <view class="list">
            ...
            <u-form :model="form" ref="uForm" label-width="auto" :error-type="errorType">
                <u-form-item label="姓名:" prop="archivesName"><u-input v-model="form.archivesName" /></u-form-item>
                <u-form-item label="性别:" prop="archivesSex">
                    <u-radio-group v-model="form.archivesSex">
                        <u-radio v-for="(item, index) in radioForm" :key="index" :name="item.sex" :disabled="item.disabled">{{ item.sex }}</u-radio>
                    </u-radio-group>
                </u-form-item>
...
import { errorType, addressRules } from '@/common/js/formrules.js' // 引入校验错误规则
export default {
    data() {
        return {
            //表单数据
    ...
    onLoad(option) {
        ...
        try {
            ...
            console.log(res)
            if (res) {
                this.form.archivesUid = res.userId
            }
        } catch (e) {
